    ...
    def intersect_nodes(self, bbox):
        ...
        res = self.cur.execute('''WITH traversal AS
                          (SELECT oid, depth, CAST(oid AS text) AS path
                           ...
                           -- Extract
                           SELECT nodes.oid, nodes.xmin, nodes.ymin, nodes.xmax, nodes.ymax, traversal.depth, traversal.path
...
if __name__ == '__main__':
    ...
    nodedata = pg.VectorData(fields=['path'])
    for node in nodematches:
        #print match
        x1,y1,x2,y2 = node[1:5] #f.bbox
        path = node[-1]
        box = {'type':'Polygon', 'coordinates':[[(x1,y1),(x1,y2),(x2,y2),(x2,y1)]]}
        nodedata.add_feature([path], box)
